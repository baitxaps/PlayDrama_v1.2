...
static CGImageRef createStarImage(CGFloat radius)
{
    ...
    CGContextRef context    = CGBitmapContextCreate((void *)NULL,
                                                    width,
                                                    height,
                                                    8,
                                                    bytesperrow,
    ...
    CGContextClearRect(context,
                       CGRectMake(0, 0, 2*radius, 2*radius)
                       );
...
- (CAEmitterLayer*) buildEmitterSnow2:(CGSize)viewBounds
{
    ...
    containerLayer.emitterCells = @[snowArray[0],snowArray[1],snowArray[3],
                                    snowArray[4],snowArray[5],snowArray[6],
                                    snowArray[7],snowArray[8],snowArray[9],
                                    snowArray[10],snowArray[11],snowArray[12]];
...
- (CAEmitterLayer*) buildEmitterFlower:(CGSize)viewBounds
{
    ...
    containerLayer.emitterCells = @[flowerArray[0], flowerArray[1],
                                    flowerArray[3], flowerArray[4],
                                    flowerArray[5], flowerArray[6],
                                    flowerArray[7]];
    parentLayer.emitterCells = @[containerLayer];
...
- (CALayer*) buildAnimatedScrollText:(CGSize)viewBounds text:(NSString*)text startPoint:(CGPoint)startPoint startTime:(NSTimeInterval)timeInterval
{
    ...
    scaleInAnimation.values             = @[[NSNumber numberWithFloat:.5f],
...
- (CALayer*) buildGradientText:(CGSize)viewBounds positon:(CGPoint)postion text:(NSString*)text
{
    ...
    groupOut.duration                   = 1;
    groupOut.animations                 = @[positionAnimationOut,boundsAnimationOut,
                                            rotateAnimationOut, opacityAnimationOut];
...
- (CAGradientLayer*) performEffectAnimation:(EffectDirection)effectDirection
{
    ...
    group.beginTime                     = 1.0;
    [group setAnimations:@[animation0, animation1, animation2,
...
{
    CGRect bounds                           = CGRectMake(viewBounds.width/2,
                                                         -viewBounds.height/2,
                                                         viewBounds.width*2,
                                                         viewBounds.height*2);
    CGFloat kRadius                         = 80;
...
- (UIImage *)imageJoint:(UIImage *)imageTarget fromImage:(UIImage *)imageOriginal
{
    ...
    float multiple = 1.5;
    [imageOriginal drawInRect:CGRectMake(
                                         (arc4random()%(int)(size.width - imageOriginal.size.width*multiple)), imageTarget.size.height - imageOriginal.size.height,
                                         imageOriginal.size.width*multiple,
                                         imageOriginal.size.height*multiple
                                         )];
...
- (UIImage*) getImageForVideoFrame:(NSURL *)videoFileURL atTime:(CMTime)time
{
    ...
    thumbnailImageRef = [assetImageGenerator copyCGImageAtTime:time
                                                    actualTime:NULL
...
- (UIImage*) getFilterImage:(UIImage*)inputImage
{
    ...
    GPULookupFilterEx *stillImageFilter         = [[GPULookupFilterEx alloc] initWithName:@"milk"
                                                                          isWhiteAndBlack:YES];
          ...
          inDuration:(double)numberOfSecondsPerFrame byFPS:(int32_t)fps
{
    ...
    [videoWriter release];
    PDebugLog(@"Finish insert image into video.");
}
...
-(CAKeyframeAnimation *)animationPageUrl:(float)duration
{
    ...
    keyframeAnimation.values                =@[[NSValue valueWithCATransform3D:transform],
                                               [NSValue valueWithCATransform3D:transform1],
                                               [NSValue valueWithCATransform3D:transform2]];
    ...
    keyframeAnimation.timingFunctions       = @[[CAMediaTimingFunction functionWithName:kCAMediaTimingFunctionEaseInEaseOut],[CAMediaTimingFunction functionWithName:kCAMediaTimingFunctionEaseIn]];
...
- (void)buildTransitionComposition:(AVMutableComposition *)composition andVideoComposition:(AVMutableVideoComposition *)videoComposition
{
    ...
    compositionVideoTracks[0] = [composition addMutableTrackWithMediaType:AVMediaTypeVideo
                                                         preferredTrackID:kCMPersistentTrackID_Invalid];
    
    compositionVideoTracks[1] = [composition addMutableTrackWithMediaType:AVMediaTypeVideo
                                                         preferredTrackID:kCMPersistentTrackID_Invalid];
    
    compositionAudioTracks[0] = [composition addMutableTrackWithMediaType:AVMediaTypeAudio
                                                         preferredTrackID:kCMPersistentTrackID_Invalid];
    
    compositionAudioTracks[1] = [composition addMutableTrackWithMediaType:AVMediaTypeAudio
                                                         preferredTrackID:kCMPersistentTrackID_Invalid];
    ...
    for (i = 0; i < [_clips count]; i++ ){
        ...
        AVAssetTrack *clipVideoTrack = [[asset tracksWithMediaType:AVMediaTypeVideo] objectAtIndex:0];
        [compositionVideoTracks[alternatingIndex] insertTimeRange:timeRangeInAsset
                                                          ofTrack:clipVideoTrack
                                                           atTime:nextClipStartTime
                                                            error:nil];
        // 视频文件可能没有音频轨道，静音
        if ([[asset tracksWithMediaType:AVMediaTypeAudio] count] != 0){
            AVAssetTrack *clipAudioTrack = [[asset tracksWithMediaType:AVMediaTypeAudio] objectAtIndex:0];
            [compositionAudioTracks[alternatingIndex] insertTimeRange:timeRangeInAsset
                                                              ofTrack:clipAudioTrack
                                                               atTime:nextClipStartTime
                                                                error:nil];
        }
        ...
        if (i+1 < [_clips count]){
            ...
            if (self.transitionType == TransitionTypeCrossFade){
                ...
                [fromLayer setTransformRampFromStartTransform:CGAffineTransformIdentity
                                               toEndTransform:CGAffineTransformMakeTranslation(-composition.naturalSize.width, 0.0)
                                                    timeRange:transitionTimeRanges[i]];
                
                // Set a transform ramp on toLayer from all the way right of the screen to identity.
                [toLayer setTransformRampFromStartTransform:CGAffineTransformMakeTranslation(+composition.naturalSize.width, 0.0) toEndTransform:CGAffineTransformIdentity
                                                  timeRange:transitionTimeRanges[i]];
            }
...
- (void)addCommentaryTrackToComposition:(AVMutableComposition *)composition withAudioMix:(AVMutableAudioMix *)audioMix
{
    ...
    [compositionCommentaryTrack insertTimeRange:CMTimeRangeMake(kCMTimeZero, commentaryTimeRange.duration)
                                        ofTrack:commentaryTrack
                                         atTime:commentaryTimeRange.start
                                          error:nil];
    ...
    AVMutableAudioMixInputParameters *trackMixComentray = \
    [AVMutableAudioMixInputParameters audioMixInputParametersWithTrack:commentaryTrack];
    
    [trackMixComentray setVolumeRampFromStartVolume:0.0f
                                        toEndVolume:0.2f
    ...
    for (i = 0; i < [tracksToDuck count]; i++){
        ...
        [trackMixComentray setVolumeRampFromStartVolume:0.5f
                                            toEndVolume:0.2f
                                              timeRange:CMTimeRangeMake(startTime, halfSecond)];
        [trackMixComentray setVolumeRampFromStartVolume:0.2f
                                            toEndVolume:0.5f
                                              timeRange:endRange];
        [trackMixArray addObject:trackMixComentray];
    }
...
- (void)buildPassThroughVideoComposition:(AVMutableVideoComposition *)videoComposition forComposition:(AVMutableComposition *)composition
{
    ...
    [[composition tracksWithMediaType:AVMediaTypeVideo] objectAtIndex:0];
