...
@interface PDramaDetailViewController () <PShareToolsDelegate,PKeyBoardViewDelegate,UITableViewDataSource,UITableViewDelegate,PDramaDelegate,SRWebSocketDelegate>
{
    AppDelegate                 *_app ;
...
{
    if (_webSocket==nil) {
        _webSocket = [[SRWebSocket alloc] initWithURLRequest:[NSURLRequest requestWithURL:[NSURL URLWithString:WBSOCKE_CONFIG_URL]]];
        _webSocket.delegate = self;
        
        PDebugLog(@"%@",@"Opening Connection...");
        [_webSocket open];
    }
}
...
- (void)addSubViews
{
    ...
    _ffmpegPlayer.barrageTextBlock = ^(NSString *string){
...
{
    PDebugLog(@"Websocket Connected");
   // self.title = @"Connected!";
}

- (void)webSocket:(SRWebSocket *)webSocket didFailWithError:(NSError *)error;
{
    PDebugLog(@":( Websocket Failed With Error %@", error);
...
{
    PDebugLog(@"Received \"%@\"", message);
    [_messages addObject:[[PWBMessage alloc] initWithMessage:message fromMe:NO]];
    _ffmpegPlayer.barrageView.barrageData = _messages;
}

- (void)webSocket:(SRWebSocket *)webSocket didCloseWithCode:(NSInteger)code reason:(NSString *)reason wasClean:(BOOL)wasClean;
{
    PDebugLog(@"WebSocket closed");
...
{
    PDebugLog(@"Websocket received pong");
    NSString *aString = [[NSString alloc] initWithData:pongPayload encoding:NSUTF8StringEncoding];
    PDebugLog(@"%@",aString);
}
